...
export default function PdvPage() {
  ...
  const { selectedTableId, selectedOrderId, items, setTable, clearTable, addItem, removeItem, changeQty, clearItems } = useCartStore()

  const [searchTerm, setSearchTerm] = useState('')
  const [activeCat, setActiveCat] = useState('')
  const [paymentModal, setPaymentModal] = useState(false)
  const [payments, setPayments] = useState<{ method: PaymentMethod; value: string }[]>([{ method: 'Dinheiro', value: '' }])
  ...
  const paidSoFar = payments.reduce((s, p) => s + (parseFloat(p.value.replace(',', '.')) || 0), 0)
  const remaining = total - paidSoFar

  const createOrder = useMutation({
    mutationFn: ordersService.create,
    onSuccess: (data, tableId) => { setTable(tableId, data.id); qc.invalidateQueries({ queryKey: ['tables'] }) },
  })
  ...
  const closeOrder = useMutation({
    mutationFn: ({ orderId, pmts }: { orderId: string; pmts: { method: PaymentMethod; amount: number }[] }) =>
      ordersService.close(orderId, pmts),
    onSuccess: () => {
      clearTable(); setPaymentModal(false); qc.invalidateQueries({ queryKey: ['tables'] })
  ...
  function handleCheckout() {
    if (Math.abs(remaining) > 0.01) { showToast(`Valor informado (${fmt(paidSoFar)}) não bate com o total (${fmt(total)})`, 'warn'); return }
    if (!selectedOrderId) return
    closeOrder.mutate({ orderId: selectedOrderId, pmts: payments.map((p) => ({ method: p.method, amount: parseFloat(p.value.replace(',', '.')) || 0 })) })
  }
  ...
  if (!selectedTableId) {
    return (
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <PageHeader title="PDV / Caixa" subtitle="Selecione uma mesa ou balcão para iniciar" />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
          {tables.map((t) => {
            const sc = tableStatusColors[t.status] ?? { bg: C.bg, text: C.textMid, label: t.status }
            return (
              <Card key={t.id} hover onClick={() => {
                if (t.status === 'Livre') createOrder.mutate(t.id)
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  ...
                  <span style={{ width: 10, height: 10, borderRadius: '50%', background: sc.text, display: 'inline-block', marginTop: 3 }} />
                </div>
                <div style={{ fontSize: 12, marginBottom: 6 }}>
                  <Badge color={sc.bg} textColor={sc.text}>{sc.label}</Badge>
                </div>
                {t.currentTotal > 0 && <div style={{ fontSize: 14, fontWeight: 700, color: C.text, marginTop: 6 }}>{fmt(t.currentTotal)}</div>}
                {t.openedAt && <div style={{ fontSize: 11, color: C.textMid, marginTop: 2 }}>{t.itemCount} iten{t.itemCount !== 1 ? 's' : ''} · {fmtElapsed(t.openedAt)}</div>}
              </Card>
            )
          })}
        </div>
    ...
    <div style={{ height: '100%', display: 'flex', gap: 16, minHeight: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 12, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={() => { clearTable(); setSearchTerm(''); setActiveCat('') }} style={{ border: 'none', background: C.bg, borderRadius: 8, padding: '8px 14px', cursor: 'pointer', fontSize: 14, color: C.textMid, fontWeight: 600, fontFamily: 'inherit' }}>← Voltar</button>
          <div style={{ fontWeight: 800, fontSize: 18, color: C.text }}>{table?.label}</div>
          {table && <Badge color={tableStatusColors[table.status]?.bg ?? C.bg} textColor={tableStatusColors[table.status]?.text ?? C.textMid}>{tableStatusColors[table.status]?.label ?? table.status}</Badge>}
        </div>

        <Input placeholder="🔍  Buscar produto…" value={searchTerm} onChange={setSearchTerm} />

        {!searchTerm && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {cats.map((c) => (
              <button key={c} onClick={() => setActiveCat(c)} style={{ border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: cat === c ? C.amber : C.surface, color: cat === c ? '#fff' : C.textMid, outline: `1px solid ${cat === c ? C.amber : C.border}` }}>{c}</button>
            ))}
          </div>
        )}
        ...
        <div style={{ flex: 1, overflow: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))', gap: 8, alignContent: 'start' }}>
          {filtered.map((p) => {
            ...
            return (
              <div key={p.id} onClick={() => addItem({ productId: p.id, productName: p.name, unitPrice: p.price, quantity: 1, goesToKitchen: p.goesToKitchen })} style={{ background: inCart ? C.amberLight : C.surface, border: `1.5px solid ${inCart ? C.amber : C.border}`, borderRadius: 10, padding: '12px 14px', cursor: 'pointer', transition: 'all .15s' }}>
                ...
                {p.goesToKitchen && <div style={{ fontSize: 10, color: C.amber, marginTop: 4 }}>🍳 Cozinha</div>}
                {inCart && <div style={{ fontSize: 11, fontWeight: 700, color: C.amber, marginTop: 4 }}>× {inCart.quantity} no carrinho</div>}
              </div>
            )
          })}
        ...
        <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {payments.map((line, idx) => {
              ...
              const otherPaid = payments.filter((_, i) => i !== idx).reduce((s, p) => s + (parseFloat(p.value.replace(',', '.')) || 0), 0)
              const troco = isCash && lineVal > (total - otherPaid) + 0.005 ? lineVal - (total - otherPaid) : null
              return (
                <div key={idx} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <select value={line.method} onChange={e => setPayments(prev => prev.map((x, i) => i === idx ? { ...x, method: e.target.value as PaymentMethod } : x))} style={{ border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', fontSize: 14, color: C.text, background: C.surface, outline: 'none', fontFamily: 'inherit', cursor: 'pointer', flex: 1 }}>
                      {paymentMethods.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                    </select>
                    <div style={{ position: 'relative', flex: 1 }}>
                      <input type="number" value={line.value} onChange={e => setPayments(prev => prev.map((x, i) => i === idx ? { ...x, value: e.target.value } : x))} placeholder={suggestion ? suggestion.toFixed(2) : '0,00'} style={{ border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 14px', fontSize: 14, color: C.text, background: C.surface, outline: 'none', fontFamily: 'inherit', width: '100%' }} />
                      {suggestion && !line.value && <button onClick={() => setPayments(prev => prev.map((x, i) => i === idx ? { ...x, value: remaining.toFixed(2) } : x))} style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', border: 'none', background: C.amberLight, color: C.amber, borderRadius: 5, padding: '2px 7px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>restante</button>}
                    </div>
                    {payments.length > 1 && <button onClick={() => setPayments(prev => prev.filter((_, i) => i !== idx))} style={{ width: 32, height: 32, border: 'none', borderRadius: 8, background: C.dangerBg, color: C.danger, cursor: 'pointer', fontSize: 16, fontWeight: 700, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>}
                  </div>
                  {troco !== null && <div style={{ background: C.successBg, borderRadius: 7, padding: '7px 14px', display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 13, color: C.success, fontWeight: 600 }}>Troco (dinheiro)</span><span style={{ fontSize: 14, fontWeight: 800, color: C.success }}>{fmt(troco)}</span></div>}
                </div>
              )
            })}
            {payments.length < 4 && <button onClick={() => setPayments(prev => [...prev, { method: 'Pix', value: '' }])} style={{ border: `1.5px dashed ${C.border}`, borderRadius: 8, padding: 9, background: 'transparent', color: C.textMid, cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>＋ Adicionar forma de pagamento</button>}
          </div>
          <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {remaining > 0.005 && <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}><span style={{ color: C.danger, fontWeight: 600 }}>Faltando</span><span style={{ fontWeight: 800, color: C.danger }}>{fmt(remaining)}</span></div>}
          </div>
      ...
      {printOrderId && <KitchenTicketPrint orderId={printOrderId} onClose={() => setPrintOrderId(null)} />}
      <ToastContainer />
    </div>
  )
}
