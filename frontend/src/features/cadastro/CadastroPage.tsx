...
const ALL_PERMISSIONS = [
  ...
  { id: "Orders", label: "Pedidos", desc: "Ver e atualizar status de pedidos" },
  {
    id: "Stock",
    label: "Estoque Geral",
    desc: "Ajustar estoque de produtos e insumos",
  },
  {
    id: "BeverageStock",
    label: "Estoque Bebidas",
    desc: "Ajustar somente bebidas",
  },
  { id: "Dashboard", label: "Dashboard", desc: "Ver relatórios e gráficos" },
  {
    id: "Registration",
    label: "Cadastro",
    desc: "CRUD de produtos, mesas e insumos",
  },
          ...
          {ICON_OPTIONS.map((icon) => (
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                background: value === icon ? C.amberLight : "transparent",
                cursor: "pointer",
                fontSize: 18,
                display: "flex",
                alignItems: "center",
...
export default function CadastroPage() {
  ...
  return (
    ...
    >
      <PageHeader
        title="Cadastro"
        subtitle="Gerencie produtos, insumos, mesas e usuários"
      />
      ...
      {tab === "products" && (
        <ProductsTab search={search} showToast={showToast} />
      )}
      {tab === "supplies" && (
        <SuppliesTab search={search} showToast={showToast} />
      )}
      {tab === "tables" && <TablesTab search={search} showToast={showToast} />}
      {tab === "users" && <UsersTab search={search} showToast={showToast} />}
      {tab === "employees" && (
        <EmployeesTab search={search} showToast={showToast} />
      )}
      {tab === "categories" && (
        <CategoriesTab search={search} showToast={showToast} />
      )}
...
function EmployeesTab({
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      setModal(false);
      showToast(
        editing ? "✅ Funcionário atualizado" : "✅ Funcionário criado",
      );
    },
  });
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map((e) => (
            ...
            >
              ...
              <div style={{ flex: 1, fontWeight: 600, color: C.text }}>
                {e.name}
              </div>
              <Badge
                style={{
                  background: e.isActive ? C.successBg : C.border,
                  color: e.isActive ? C.success : C.textMid,
                }}
              >
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Btn onClick={openNew} size="sm" icon="➕">
            Novo Produto
          </Btn>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 24 }}>
            ...
            >
              <input
                type="checkbox"
                checked={form.goesToKitchen}
                onChange={(e) =>
                  setForm((f) => ({ ...f, goesToKitchen: e.target.checked }))
                }
                style={{
                  width: 16,
                  height: 16,
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, hasStock: e.target.checked }))
                }
                style={{
                  width: 16,
                  height: 16,
          ...
          >
            ...
            <Btn
              onClick={() => save.mutate()}
              disabled={save.isPending || !form.name || !form.categoryId}
            >
              {save.isPending
                ? "Salvando…"
                : editing
...
function SuppliesTab({
  ...
  const qc = useQueryClient();
  const { data: supplies = [] } = useQuery<SupplyDto[]>({
    queryKey: ["supplies"],
    queryFn: async () => (await api.get("/supplies")).data,
  });
  const { data: supplyCategories = [] } = useQuery<SupplyCategoryDto[]>({
    queryKey: ["supply-categories"],
    queryFn: async () => (await api.get("/supply-categories")).data,
  });
  const filtered = supplies.filter(
    (s) => !search || s.name.toLowerCase().includes(search.toLowerCase()),
  );

  const catOptions = supplyCategories.map((c) => ({
    value: c.slug,
    label: `${c.icon} ${c.name}`,
  }));
  ...
  function openNew() {
    setForm({ ...emptySupply, categorySlug: defaultCat });
    setEditing(null);
    setModal(true);
  }
  function openEdit(s: SupplyDto) {
    setForm({
      ...
      minimumQuantity: String(s.minimumQuantity),
      supplier: s.supplier ?? "",
    });
    setEditing(s.id);
    setModal(true);
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? api.put(`/supplies/${editing}`, payload)
        : api.post("/supplies", payload);
    },
    onSuccess: () => {
          ...
          >
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7}>
                    <Empty icon="🧂" msg="Nenhum insumo encontrado" />
                  </td>
                </tr>
              )}
            </tbody>
        ...
        >
          ...
          >
            <Select
              ...
              onChange={(v) => setForm((f) => ({ ...f, categorySlug: v }))}
              options={catOptions.length ? catOptions : SUPPLY_CATS}
            />
            <Select
              label="Unidade"
              value={form.unit}
          ...
          >
            ...
            <Input
              ...
              onChange={(v) => setForm((f) => ({ ...f, minimumQuantity: v }))}
              placeholder="0"
            />
            <Input
              label="Custo por un (R$)"
              value={form.costPerUnit}
              onChange={(v) => setForm((f) => ({ ...f, costPerUnit: v }))}
              placeholder="0,00"
            />
          </div>
          <Input
...
function TablesTab({
  ...
  const { data: tables = [] } = useQuery<TableDto[]>({
    ...
    queryFn: async () =>
      (await api.get(`/tables?includeInactive=${showInactive}`)).data,
  });
      ...
      >
        {filtered.map((t) => {
          ...
          return (
            <Card
              key={t.id}
              style={{ padding: 18, opacity: t.isActive ? 1 : 0.5 }}
            >
              <div
                style={{
                  display: "flex",
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button
                  ...
                  style={{
                    flex: 1,
                    border: "none",
                    background: t.isActive ? C.dangerBg : C.successBg,
                    borderRadius: 6,
                    padding: "5px 0",
                    fontSize: 12,
                    cursor: "pointer",
                    fontFamily: "inherit",
                    color: t.isActive ? C.danger : C.success,
                    fontWeight: 600,
                  }}
                >
        ...
        {filtered.length === 0 && (
          <div style={{ gridColumn: "1 / -1" }}>
            <Empty icon="🪑" msg="Nenhuma mesa encontrada" />
          </div>
        )}
      </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Btn
              onClick={() => save.mutate()}
              disabled={save.isPending || !form.label || !form.number}
            >
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 8 }}>
        {(
          [
            ["products", "📦 Categorias de Produtos"],
            ["supplies", "🧂 Categorias de Insumos"],
          ] as const
        ).map(([v, l]) => (
          <button
            key={v}
            onClick={() => setSub(v)}
            style={{
              border: `1px solid ${sub === v ? C.amber : C.border}`,
              background: sub === v ? C.amber : C.surface,
              borderRadius: 8,
              padding: "7px 16px",
              fontSize: 13,
              fontWeight: 600,
      ...
      {sub === "products" && (
        <ProductCategoriesTab search={search} showToast={showToast} />
      )}
      {sub === "supplies" && (
        <SupplyCategoriesTab search={search} showToast={showToast} />
      )}
    </div>
  );
}
...
function ProductCategoriesTab({
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? api.put(`/categories/${editing}`, payload)
            ...
            <tbody>
              {filtered.map((c) => (
                <tr
                  ...
                  style={{ borderBottom: `1px solid ${C.border}` }}
                >
                  <td style={{ padding: "10px 16px", fontSize: 22 }}>
                    {c.icon}
                  </td>
                  <td
                    style={{
                  ...
                  <td style={{ padding: "10px 16px", color: C.textMid }}>
                    {c.sortOrder}
                  </td>
                  <td style={{ padding: "10px 16px" }}>
                    <button
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5}>
                    <Empty icon="🏷️" msg="Nenhuma categoria encontrada" />
                  </td>
                </tr>
              )}
            </tbody>
        ...
        >
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <label
                style={{ fontSize: 13, fontWeight: 600, color: C.textMid }}
              >
                Slug
              </label>
              <div
...
function SupplyCategoriesTab({
  ...
  const qc = useQueryClient();
  const { data: categories = [] } = useQuery<SupplyCategoryDto[]>({
    queryKey: ["supply-categories"],
    queryFn: async () => (await api.get("/supply-categories")).data,
  });
  const filtered = categories.filter(
  ...
  const save = useMutation({
    mutationFn: () => {
      const payload = { ...form, sortOrder: parseInt(form.sortOrder) || 0 };
      return editing
        ? api.put(`/supply-categories/${editing}`, payload)
        : api.post("/supply-categories", payload);
    },
    onSuccess: () => {
  ...
  return (
    <>
          ...
          >
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5}>
                    <Empty icon="🧂" msg="Nenhuma categoria encontrada" />
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <Modal
        open={modal}
        onClose={() => setModal(false)}
        title={editing ? "Editar Categoria" : "Nova Categoria de Insumo"}
        width={440}
      >
        <div
          style={{
...
function UnitsTab({
  ...
  const qc = useQueryClient();
  const { data: units = [] } = useQuery<UnitDto[]>({
    queryKey: ["units"],
          ...
          >
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={4}>
                    <Empty icon="📐" msg="Nenhuma unidade encontrada" />
                  </td>
                </tr>
              )}
            </tbody>
      ...
      >
        ...
        >
          ...
          >
            ...
            <Btn
              onClick={() => save.mutate()}
              disabled={save.isPending || !form.name || !form.label}
            >
              {save.isPending
                ? "Salvando…"
                : editing
          ...
          >
            ...
            <tbody>
              {filtered.map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "11px 16px" }}>
                      <Badge color={rc.bg} textColor={rc.text}>
                        {u.role}
                      </Badge>
                    </td>
                    <td style={{ padding: "11px 16px" }}>
                      ...
                      >
                        ...
                        {u.permissions.length === 0 && (
                          <span style={{ color: C.textLight, fontSize: 13 }}>
                            Sem acesso
                          </span>
                        )}
                      </div>
                    ...
                    <td style={{ padding: "11px 16px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button
                          ...
                          style={{
                            border: "none",
                            background: u.isActive ? C.dangerBg : C.successBg,
                            borderRadius: 6,
                            padding: "4px 10px",
                            fontSize: 12,
                            cursor: "pointer",
                            fontFamily: "inherit",
                            color: u.isActive ? C.danger : C.success,
                            fontWeight: 600,
                          }}
                        >
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={5}>
                    <Empty icon="👥" msg="Nenhum usuário encontrado" />
                  </td>
                </tr>
              )}
            </tbody>
            ...
            >
              {ROLES.map((r) => (
                <button
                  ...
                  style={{
                    border: `1px solid ${form.role === r.value ? C.amber : C.border}`,
                    background:
                      form.role === r.value ? C.amberLight : C.surface,
                    borderRadius: 8,
                    padding: "5px 12px",
                    fontSize: 12,
                    fontWeight: 600,
                    cursor: "pointer",
                    fontFamily: "inherit",
                    color: form.role === r.value ? C.amber : C.textMid,
                  }}
                >
                  {r.label}
                </button>
              ...
              {ALL_PERMISSIONS.map((p) => (
                <label
                  ...
                  style={{
                    ...
                    padding: "8px 10px",
                    borderRadius: 8,
                    border: `1px solid ${form.permissions.includes(p.id) ? C.amber : C.border}`,
